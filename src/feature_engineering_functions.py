...
def apply_productcd_day_count(transaction):
    ...
    for code in ['W', 'C', 'R', 'H', 'S']:
        ...
        grouped_dict[code] = dict(zip(zip(subset['ProductCD'], subset['DT_D']), subset['isFraud']))
    ...
    for code in ['W', 'C', 'R', 'H', 'S']:
        key = (code, dt_d)
        if product_cd == code:
            transaction[f'ProductCD_{code}_Day'] = grouped_dict.get(code, {}).get(key, 0)
        else:
...
def compare_and_clean_columns(transaction_df):
    ...
    # Convertir 'input_columns' en set et identifier les colonnes inattendues
    ...
    # Réorganiser les colonnes dans le même ordre que dans expected_columns
    # en gardant uniquement celles qui sont présentes dans transaction_df
    ...
    # Retourner à la fois le DataFrame nettoyé et les informations
